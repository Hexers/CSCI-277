...
int stopwatchdisplay()
{
	/* This code is repurposed from Challenge Problem 5.1 - Aleksandar Kljaic */
	/* https://github.com/Hexers/CIS-276-Fall2016/blob/master/Challenge%20Problem%205.1/Stopwatch.cpp */

	/* Variables */
	int petrolMilliseconds = 0; /* Initalizing Milliseconds */
	...
	int petrolSeconds = 0;
		...
		do /* Same as while(true) */
		{
			system("CLS"); /* Clears Screen - Could have used clrscr since conio.h is called but whatever */		
			if(petrolMilliseconds == 10) /* If milliseconds are 10 increment seconds and reset milliseconds to zero */
			{
				++petrolSeconds;
				petrolMilliseconds = 0;
			}
			cout << "Gallons Dispensed: " << petrolSeconds << "." << petrolMilliseconds << endl; /* Outputs the following: s:ms */
			++petrolMilliseconds; /* Increment milliseconds once all other if statements are met */
			Sleep(100); /* Sleeps for 100 milliseconds */

		} /* For Loop Ends */
		while(petrolSeconds == petrolAmount); /* While Exception */
		{
			system("PAUSE");
			exit(0);
		} /* While Exception Ends */
	}
